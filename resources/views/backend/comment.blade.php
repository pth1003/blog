...
@section('comment')
    ...
    <div class="d-flex align-items-center justify-content-between w-30">
        ...
        <a class="btn btn-success btn-sm" href="{{ route('backend.comment.list', ['status'=>0]) }}">Pending {{$countCmtPending}} <i class="bi bi-hourglass-bottom"></i></a>
        <a class="btn btn-warning btn-sm text-white" href="{{ route('backend.comment.list', ['status'=>1]) }}">Solved {{$countCmtSolved}} <i class="bi bi-cloud-check-fill"></i></a>
    </div>
